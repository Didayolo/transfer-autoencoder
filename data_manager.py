...
def processing(tensor_4d, label, shape, padding=False):
    """
    Process data shape.
    Resize and/or pad tensor to a certain shape e.g. (1, 52, 52, 3).
    """
...
def input_fn(dataset, shape, perform_shuffle=False, batch_size=1, padding=False):
    """ Input function to apply on tf dataset.
    """
...
def get_example(input_dir, basename, shape, padding=False, train_test_split=False, test=True):
    """ Load a dataset and return a batch.
    """
    # Load a dataset
